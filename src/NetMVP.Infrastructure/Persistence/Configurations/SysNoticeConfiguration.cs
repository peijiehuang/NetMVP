...
public class SysNoticeConfiguration : IEntityTypeConfiguration<SysNotice>
{
    public void Configure(EntityTypeBuilder<SysNotice> builder)
    {
        ...
        builder.Property(e => e.NoticeType).HasColumnName("notice_type").HasMaxLength(1).HasDefaultValue(NoticeConstants.NOTICE_TYPE_NOTICE);
        builder.Property(e => e.NoticeContent).HasColumnName("notice_content");
        builder.Property(e => e.Status).HasColumnName("status").HasMaxLength(1).HasDefaultValue(NoticeConstants.NOTICE_STATUS_NORMAL);
